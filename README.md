...
- Public forms (Get a Quote, Become a Dealer, Parts Request, Request Access) post to `submit_lead()` in Supabase, which stores the enquiry in `leads` and emails the office through Resend (docs/supabase-leads.sql). The destination address is fixed in app_settings, so nothing a visitor types can redirect the mail. A honeypot field named `trr_hp`, deliberately meaningless because the original `company_website` was matched by Chrome autofill and silently ate real enquiries, a 45 second duplicate guard, and a 12 per minute cap keep bots off it. A honeypot hit is stored and flagged rather than discarded, kept out of `lead_summary` but visible in `lead_flagged`, because the cost of a false positive is a customer who believes they got in touch and never hears back. If the call fails or times out the page falls back to the old mailto route, so a lead is never silently dropped
...
- The office request list is filtered, not a feed. It opens on `new` (submitted only), with tabs for in-progress, finished, and everything, counts on each, a search across request number, dealership, and contact name, and 25 at a time with Show more. `admin_recent_requests(bucket, search, limit, offset)` does the filtering server side and returns a counted `more` rather than inferring it from a full page
...
- **Never prefix a class or id with `ad-`.** EasyList ships generic cosmetic rules including `###ad-new` and `##.ad-card`, so any visitor running uBlock, AdBlock Plus, AdGuard, or Brave shields has that element hidden before they see it. The office page shipped with `id="ad-new" class="ad-card"` on its main form and the form was invisible to anyone with a blocker, with the markup present and the server fine. The prefix is now `od-`. `docs/stamp-assets.py` fails the build on `ad-`, `ads-`, `advert`, `banner-`, `sponsor`, `promo-`, `popup-`, and friends
